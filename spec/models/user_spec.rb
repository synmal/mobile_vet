...
RSpec.describe User, type: :model do
  context 'sign up with same email' do
    it 'should give error' do
        user1 = User.create!(email: 'test@example.com', name: 'John Smith', address: 'test', password: '123')
        expect{ User.create!(email: 'test@example.com', name: 'John Smith', address: 'test', password: '123') }.to raise_error(ActiveRecord::RecordInvalid)
    end
  end
